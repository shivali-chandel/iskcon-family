      ...
      .then(async (result) => {
        if (result) {
          let selected =  await Setting.findOne({
           ...
           Setting.update(setting, { where: { id : 30 } })
           .then((result) => {
             if (result) {
...
addNewTransactions = (field_name, current_data) => {
  console.log('field_name', field_name, 'curr', current_data);
  if (field_name === null) {
    return current_data;
  } else {
  ...
  transactionLineItems.map((item) => {
    ...
    if (typeof arabic_book_type_points[book_type] !== "undefined") {
      if(item.item.language == 'Arabic'){
       total_arabic_points +=
        item.transaction_book_points * parseInt(item.quantity);
        arabic_book_type_points[book_type] = total_arabic_points;
      }
    }
  ...
  if (transactions && transactions.length > 0) {
    ...
    const periods = await Period.findAll({
      ...
      const groups = await Group.findAll({
        attributes:["id","book_distribution_reporting_level"]
      })
      .then((result) => {
        return result;
      })
      ...
      if (periods && periods.length > 0) {
        ...
        transactions.map((trans) => {
          ...
          trans.transaction_people.map((people) => {
            ...
            }else if(groupdata[group_id].reporting_level == 'Group'){
              if(typeof transaction[group_id]["distributors"][people.distributor_id] === "undefined"){
                transaction[group_id]["distributors"][people.distributor_id]={};
                transaction[group_id]["distributors"][people.distributor_id]['period']={};
              }
...
router.get("/topten-temple-by-continent", async function (req, res) {
  ...
  let period=null;
  if(query_param.period_id !== undefined){
     period=query_param.period_id;
  }
  let thisYearData = await topTempleByContinent(year,period);
  for (let continent in thisYearData) {
    thisYearData[continent] = thisYearData[continent].sort(
      (a, b) => b.total - a.total
    );
  }
  const lastYearData = await topTempleByContinent(parseInt(year) - 1,period);
  const formattedResult = {};
  for (let continent in thisYearData) {
...
getTopTenCountriesCumulative = async (year, period_id) => {
  ...
  let groupByCountry = {};
  // console.log('aaa', allCountriesSummary.length);
  allCountriesSummary.map((summary) => {
      summary = JSON.parse(JSON.stringify(summary));
      if (typeof groupByCountry[summary.group.meeting_country] === "undefined") {
        groupByCountry[summary.group.meeting_country] = {
...
router.get("/newsletter-topten-countries-cumulative", async function (
  ...
  if(query_param.period_id !== undefined){
    ...
    const this_year_period_data=await getPeriodData(_where);
    let last_year=parseInt(year) - 1;
    where= {year:last_year.toString(),type: this_year_period_data.type,number:this_year_period_data.number,is_active : 1};
    const last_year_period_data=await getPeriodData(_where); 
    if(last_year_period_data != null){
      last_year_period=last_year_period_data.id;
    } 
...
router.get("/newsletter-topten-temples-size", async function (
  ...
  mahaSmallTemples = sortArrayAndReturnTenResults(mahaSmallTemples);
  smallTemples = sortArrayAndReturnTenResults(smallTemples);
  mediumTemples = sortArrayAndReturnTenResults(mediumTemples);
  largeTemples = sortArrayAndReturnTenResults(largeTemples);
  return res.status(200).send({mahaSmallTemples, smallTemples, mediumTemples, largeTemples});
...
getAllTemples = async (year, period,field, param) => {
  let _where = {}
  if(param !== undefined) {
    _where={
      year: {
...
router.get("/newsletter-all-temples", async function (req, res) {
  ...
  if(query_param.param !== undefined){
    thisYearData = await getAllTemples(year, period_id,"actual_book_points_book_type_total", "c");
  }
  else {
    thisYearData = await getAllTemples(year, period_id,"actual_book_points_book_type_total");
  }
  ...
  if(period_id != null && last_year_period == null ){
    ...
    if(query_param.param !== undefined){
      lastYearData = await getAllTemples(parseInt(year) - 1, last_year_period,"actual_book_points_book_type_total", "c");
    }
    else {
      lastYearData = await getAllTemples(parseInt(year) - 1, last_year_period,"actual_book_points_book_type_total");
    }
  }
...
router.get("/newsletter-topten-temple-arabic", async function (req, res) {
  ...
  const thisYearData = await getAllTemples(year, period,"actual_arabic_book_points_book_type_total");
  const thisYearCountryData = [];
  for (let temple in thisYearData) {
    // console.log('templesData', thisYearData[temple].points, 'type', typeof thisYearData[temple].points);
    thisYearCountryData.push({
  ...
  const lastYearData = await getAllTemples(parseInt(year) - 1, period,"actual_arabic_book_points_book_type_total");
  const lastYearCountryData = [];
  for (let temple in lastYearData) {
    lastYearCountryData.push({
      title: thisYearData[temple].title,
...
let summaryBookTypeWiseYearly = await BusinessPlanSummary.findAll({
  ...
  let month_points =summaryBookTypeWise.actual_book_points_total !== null
  ? summaryBookTypeWise.actual_book_points_total
  : 0;
...
//       {
//         model: TransactionLineItem,
//         attributes: ["id", "item_id", "quantity", "net_amount"],
//         include: [
//           {
//             model: Item,
...
//         )
//       : 0;
//     let change=start_date ? "-": last_year_points === 0
//     ? "-"
...
router.get("/newsletter-top-individuals", async function (req, res) {
  ...
  let period=null;
  let people_transaction = [];
  let last_year_people_transaction = [];

  if(query_param.period_id !== undefined){
    period=query_param.period_id;
  }
  if(query_param.param !== undefined){
    people_transaction = await getTopIndividuals(year,period, 'c');
    last_year_people_transaction = await getTopIndividuals(
      parseInt(year) - 1, period, 'c'
    );
  }
  else {
    people_transaction = await getTopIndividuals(year,period);
    last_year_people_transaction = await getTopIndividuals(
      parseInt(year) - 1, period
    );
  }
  ...
  formattedResult = formattedResult.sort((a, b) => b.points - a.points);
  formattedResult = formattedResult.slice(0, 50);
  return res.status(200).send(formattedResult);
});

getTopIndividuals = async (year,period,param) => {
  let _where = {}
  if(param !== undefined) {
    _where={
      year: {
...
router.patch("/update-settings-data", async function (req, res){
  ...
  {
    //local 
   // const settings = await Setting.findOne({ where: { id : 11 } });
    //live
    const settings = await Setting.findOne({ where: { id : 30 } });
