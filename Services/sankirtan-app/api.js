...
router.get("/validate-host1", async function (req, res) {
 console.log('my request', req.session.headerData);
  if (
          ...
          People.findOne({
            ...
            .then((user) => {
              if (user) {
                ...
                .then((array) => {
                  console.log('reachedddd', settingsFormatted)
                  if(array)
                  {
                  res.status(200).send({
                    organization: org,
...
router.post("/people", async function (req, res) {
  ...
  for(people in req.body){
   ...
   sendPeopleEmail(req.body[people].email)
    }
    catch
    {
...
router.get("/search-books", function (req, res) {
  ...
  Item.findAll({
    attributes: ["id", "name"],
    where: { item_type: "Book",status : "1", name: { [Sq.Op.iLike]: `%${q}%` } },
  })
    .then((items) => {
      if (items) {
        res.status(200).send(items);
...
checkUpdateStatus = async (
  ...
  let organization = await Organization.findOne({ where: { id : organization_id} });  
  let period  = await Period.findOne({ where: { id : period_id} });
  let status='true';
  if(organization.goal_setting_period_type != '' && organization.goal_setting_period_type=='MSF' && period.type=='Month'){
    status='false';
  }
  if(organization.goal_setting_period_type != '' && organization.goal_setting_period_type=='Month' && period.type=='MSF'){
    status='false';
  }
  return status;
}
router.get("/goal-setting", async function (req, res) {
  const { group_id, year, distributor_id, period_id,organization_id } = req.query;
  ...
  // if (period_id) {
    ...
    let org_goal_setting_type='';
    if(organization_id !== undefined){
      org_goal_setting_type =await checkUpdateStatus({organization_id:organization_id,period_id});
   }
...
const rollupGoals = async (
  ...
  console.log('goal_book_points',goal_book_points);
  console.log('goal_amount_remitted_bbt',goal_amount_remitted_bbt);
  console.log('goal_amount_collected',goal_amount_collected);
  console.log('period_id',period_id);
  console.log('yearly_update',yearly_update);
  console.log('existing_data',existing_data);
  const rollup_data =await BusinessPlanSummary.findOne({ where: _where });
  if(rollup_data == null){
    let goal = {
      year: year,
      period_id:period_id,
    ...
    let goal = {};
      goal = {
        ...
        book_points_goal_roll_up: 
          adjustExistingGoal(
            rollup_data.book_points_goal_roll_up,
            goal_book_points,
        ...
        bbt_amount_goal_roll_up: 
          adjustExistingGoal(
            rollup_data.bbt_amount_goal_roll_up,
        ...
        group_amount_goal_roll_up: 
          adjustExistingGoal(
            rollup_data.group_amount_goal_roll_up,
        ...
        book_points_goal_total:
          adjustExistingGoal(
            rollup_data.book_points_goal_total,
            goal_book_points,
            existing_data.old_book_point_goal
              ? existing_data.old_book_point_goal
              : 0,
        ...
        bbt_amount_goal_total: 
          adjustExistingGoal(
            rollup_data.bbt_amount_goal_total,
            goal_amount_remitted_bbt,
            existing_data.old_bbt_amount_goal
              ? existing_data.old_bbt_amount_goal
              : 0,
        ...
        group_amount_goal_total:
          adjustExistingGoal(
            rollup_data.group_amount_goal_total,
            goal_amount_collected,
            existing_data.old_group_amount_goal
              ? existing_data.old_group_amount_goal
              : 0,
      ...
      console.log('roll-up goals',goal);
      await BusinessPlanSummary.update(goal, { where: { id: rollup_data.id } });
  }
  if(yearly_update == 'true'){
    await updateYearly(
      {
        group_id: group_id,
        year,
...
checkPeriodWithDays = async (
  ...
  const periods = await Period.findAll({
    ...
    let periods_data={};
    let p_start = new Date(start);
    let p_end = new Date(end);
    periods.map((period) => {
      let period_start = new Date(period.start);
      let period_end = new Date(period.end);
      if ( 
        period_start.getTime() >= p_start.getTime() &&
        period_start.getTime() <= p_end.getTime()
      ) {
        let diff_start=(p_start >= period_start) ? p_start : period_start;
        let diff_end=(p_end >= period_end) ? period_end : p_end;
        let time_difference = diff_end.getTime() - diff_start.getTime();  
        let days_difference = time_difference / (1000 * 60 * 60 * 24); 
       
        periods_data[period.id]=days_difference + 1;
      }
      if ( 
        period_end.getTime() >= p_start.getTime() &&
        period_end.getTime() <= p_end.getTime()
      ) {
...
router.post("/goal-setting", async function (req, res) {
 let goalData = req.body;
  if (typeof goalData.period_id !== "undefined") {
    ...
    const goal_period_id=period.id;
    let allPeriods=await checkPeriodWithDays({start:period.start,end:period.end});
    ...
    if (
      ...
      _where = { ..._where, distributor_id: goalData.distributor_id };
    }else{
      _where = { ..._where, distributor_id: null };
    }

    const summary_data = await BusinessPlanSummary.findOne({ where:  _where  });
    let existing_summary_data = {};
    if (summary_data != null) {
      existing_summary_data = {
        old_book_point_goal: summary_data.book_points_goal,
        old_bbt_amount_goal: summary_data.bbt_amount_goal,
        old_group_amount_goal: summary_data.group_amount_goal,
      }; 
   }
    ...
    let points_array={};
    let i=1;
    let total_book_points_goal=0;
    let total_bbt_amount_goal=0;
    let total_group_amount_goal=0;
    let len_array=Object.keys(allPeriods).length
    for (let period in allPeriods) {
      if(goal_period_id != period){
        let book_points_goal=(i == len_array) ? ((goalData.goal_book_points - total_book_points_goal).toFixed(2)) : (allPeriods[period]/allPeriods[goal_period_id] * goalData.goal_book_points).toFixed(2);
        let bbt_amount_goal=(i == len_array) ? ((goalData.goal_amount_remitted_bbt - total_bbt_amount_goal).toFixed(2)) : (allPeriods[period]/allPeriods[goal_period_id] * moneyToNumber(goalData.goal_amount_remitted_bbt)).toFixed(2);
        let group_amount_goal=(i == len_array) ? ((goalData.goal_amount_collected - total_group_amount_goal).toFixed(2)) : (allPeriods[period]/allPeriods[goal_period_id] * moneyToNumber(goalData.goal_amount_collected)).toFixed(2);
        points_array[period]={ book_points_goal,bbt_amount_goal,group_amount_goal};
        total_book_points_goal += +book_points_goal;
        total_bbt_amount_goal += +bbt_amount_goal;
        total_group_amount_goal += +group_amount_goal;
      }else{
        points_array[period]={ "book_points_goal":goalData.goal_book_points,"bbt_amount_goal":goalData.goal_amount_remitted_bbt,"group_amount_goal":goalData.goal_amount_collected};
      }
      i++;
    }
    ...
    for (let period in allPeriods) {
      let period_id=period;
       let yearly_update = (goal_period_id == period_id) ? 'true' : 'false';
      _where = {
        ..._where,
        period_id,
      }
      ...
      let goal_setting_data = await BusinessPlanSummary.findOne({ where: _where });
      let goal_book_points=points_array[period_id].book_points_goal;
      let goal_amount_remitted_bbt=  points_array[period_id].bbt_amount_goal;
      let goal_amount_collected=  points_array[period_id].group_amount_goal; 
      if (goal_setting_data != null) {
        existing_data = {
          old_book_point_goal: goal_setting_data.book_points_goal,
          old_bbt_amount_goal: goal_setting_data.bbt_amount_goal,
          old_group_amount_goal: goal_setting_data.group_amount_goal,
        };
        goal = {
          ...goal,
          // end old summary
          book_points_goal:adjustExistingGoal(
            goal_setting_data.book_points_goal,
            goal_book_points,
            existing_data.old_book_point_goal,
          ), 
          bbt_amount_goal:adjustExistingGoal(
            goal_setting_data.bbt_amount_goal,
            goal_amount_remitted_bbt,
            existing_data.old_bbt_amount_goal,
          ),
          group_amount_goal:adjustExistingGoal(
            goal_setting_data.group_amount_goal,
            goal_amount_collected,
            existing_data.old_group_amount_goal,
          ),
          book_points_goal_total: 
            adjustExistingGoal(
              goal_setting_data.book_points_goal_total,
              goal_book_points,
              existing_data.old_book_point_goal,
          ),
          bbt_amount_goal_total: 
            adjustExistingGoal(
              goal_setting_data.bbt_amount_goal_total,
              goal_amount_remitted_bbt,
              existing_data.old_bbt_amount_goal,
          ),
          group_amount_goal_total: 
            adjustExistingGoal(
              goal_setting_data.group_amount_goal_total,
              goal_amount_collected,
              existing_data.old_group_amount_goal,
            ),
        };
        ...
        await BusinessPlanSummary.update(goal, { where: { id: goal_setting_data.id } });  
      }
      else{
        goal = {
          ...goal,
      ...
      if (typeof goalData.distributor_id !== "undefined" && goalData.distributor_id > 0) 
      {
        DistributionGoals(
        {
          ...
          year: goalData.year.toString(),
          period_id: period_id,
          created_by_id: goalData.created_by_iupdateYearlyn_id,
        },
        {
          goal_book_points,
          goal_amount_remitted_bbt,
...
const adjustExistingGoal = (current, coming, old) => {
  if (current) {
    ...
    let result= parseFloat(current_test) - parseFloat(old_test) + parseFloat(coming_test);
    return result;
  } else {
    return coming;
  }
...
router.get("/group-by-member-id/:member_id", function (req, res) {
  const { member_id } = req.params;
  People.findOne({attributes: ['role'], where: { id: member_id } }).then((peopleresult) => {
    if (peopleresult) {
        GroupMember.findAll({
          where: {
            people: member_id,
          ...
          .then((result) => {
            if (result) {
              ...
              if(peopleresult.role != 'Super Admin'){
                 _where = { ..._where, id: { [Sq.Op.in]: group_ids }, };
              }
              Group.findAll({
                attributes: [
                  "id",
...
router.post("/book-distribution", function (req, res) {
  ...
  upload(req, res, function (err) {
    ...
    const transaction_book_points_arr2 = itemData.bbt_point.map(
      (point, indx) => point
    );
    ...
    console.log('arr', transaction_book_points_arr)
    const transaction_book_points2 = getArrayValueSum(
      transaction_book_points_arr
    );
    const userId = JSON.parse(req.body.userId);
    itemData = {
 ...
 //   console.log('itemData', itemData.bookdistributors.toString());
    ...
    .then((result) => {
      if (result) {
        ...
        // Add People
        if(itemData.people_first_name != '' && itemData.people_email != ''){
          let people = {
          ...
          itemData.distributors.map((dist) => {
            distributors.push(dist.name)
          });
          people = {...people, notes : JSON.stringify({'referred_by' : distributors.toString(), 'money_donated' : itemData.net_amount, 'books_purchased' : bookpurchased.toString(), 'initiation_level' : itemData.people_initiation_level })}
        }
       ...
       // Add or Update Transaction Line Items
        if (itemData.item.length) {
          ...
          let i=1;
          let len_array=Object.keys(itemData.item).length;
          itemData.item.map((itm, row) => {
            // console.log('singlebbt', itemData.bbt_point[row],'total_points', transaction_book_points2, 'total_amount', itemData.total_amount)
            let price2 = itemData.amount[row]/itemData.quantity[row]
            let net = price2*itemData.quantity[row]
            let net2 = (itemData.bbt_point[row]*itemData.quantity[row]/transaction_book_points);
            if(i == len_array){
              let tot_sum= +sum;
              net_amount = (itemData.total_amount - tot_sum).toFixed(2) ;
              console.log();
            }else{
              net_amount = (net2*itemData.total_amount).toFixed(2);
            }
            
            let price = +net_amount/itemData.quantity[row]
            sum += +net_amount;
            

            // net_amount = row == 1 ? getRoundedAmount(net_amount) : net_amount
            // console.log('net2',row, 'rem', remaining, net_amount, 'sum', parseFloat(sum).toFixed(2), 'sub', parseFloat(itemData.total_amount) - getRoundedAmount(sum), 'tot', itemData.total_amount, 'sum', sum)
              ...
              addUpdateTransactionLineItem(
                {
                  ...
                  price: itemData.isTotalAmount ? price : price2,
                  net_amount : itemData.isTotalAmount ?  net_amount : itemData.amount[row],
...
getRoundedAmount = (amount) => {
    var with2Decimals = amount.toString().match(/^-?\d+(?:\.\d{0,2})?/)[0]
    ...
    Transaction.update(itemData, { where: { id: id } })
      .then((result) => {
        ...
        let remaining = 0
            ...
            let net = price2*itemData.quantity[row]
          let net2 = (itemData.bbt_point[row]*itemData.quantity[row]/transaction_book_points)
          let net_amount = (net2*itemData.total_amount)
          let price = net_amount/itemData.quantity[row]
          sum += net_amount
          if(row == 1){
             remaining = parseFloat(itemData.total_amount) - getRoundedAmount(sum)
          }
          // sum += net_amount
          // if(row == 1){
          //   net_amount = itemData.total_amount - sum
          // }
         // net_amount = row == 1 ? getRoundedAmount(net_amount) : net_amount
         console.log('net2', net_amount)
              if (+itemData.quantity[row] > 0) {
                addUpdateTransactionLineItem(
                  {
                    transaction_id,
                    ...
                    quantity: itemData.quantity[row],
                      price: itemData.isTotalAmount ? price : price2,
                 net_amount : itemData.isTotalAmount ? row ==2 ? remaining : net_amount : itemData.amount[row],
                    transaction_book_points: itemData.bbt_point[row],
                    organization_id,
                  },
                  { transaction_id, item_id: itm }
          ...
          if (
            ...
            TransactionPeople.destroy({ where: { transaction_id: transaction_id } })
            .then((result) => {
              itemData.distributors.map((dist) => {
                let distributor_id=dist.id;
                let distributor_index=dist.index;
...
//  return false;
 // console.log('receiveremail', email);
  ...
  for(let setting in settings){
    if(settings[setting].id == 7){
      user = settings[setting].value
    }
    else if (settings[setting].id == 10){
  ...
  console.log('hereee', user, password, host)
                  ...
                  await Transaction.create(itemData)
                    .then(async (result) => {
                      ...
                      //adding notes field
                        ...
                        if (people.organization_id != 1 && (itemData.people_first_name != '' && itemData.people_email != '')) {
                        ...
                        if (distributor_ids.length > 0) {
                          distributor_ids.map((distributor_id,distributor_index) => {
...
bookTypePoints = (transactionLineItems,total_amount,team_type) => {
  ...
  transactionLineItems.map((item) => {
    ...
    if (typeof book_type_points[book_type] !== "undefined") {
      ...
      // {
      //   transaction_amount = parseFloat(moneyToNumber(item.net_amount));
      // }
    ...
    if (typeof arabic_book_type_points[book_type] !== "undefined") {
      if(item.item.language == 'Arabic'){
        ...
        item.transaction_book_points * parseFloat(item.quantity);
      }
    }
   
  });

  return { book_type_points, transaction_book_points ,transaction_book_quantity ,total_amount,arabic_book_type_points};
};
        ...
        transactions.map((trans) => {
          ...
          checkperiod.map((value) => {
            ...
            trans = {
              ...
              ...bookTypePoints(trans.transaction_line_items,trans.total_amount,groupdata[group_id].sankirtan_group_type),
            };
          ...
          trans.transaction_people.map((people) => {
            if(groupdata[group_id].reporting_level == 'Individual' && people.distributor_index == 0){
              ...
              checkperiod.map((value) => {
                ...
                trans = {
                  ...JSON.parse(JSON.stringify(trans)),
                  ...bookTypePoints(trans.transaction_line_items,trans.total_amount),
                };
...
combineCollectionss = (arr_data) => {
  ...
  Object.keys(arr_data).map((i) => {
    if (typeof arr_data[i] !== "undefined") {
      actual_bbt_amount  += (+ arr_data[i].total_amount);
...
router.get("/top-ten-group", async function (req, res) {
  ...
  let _where = {
    reports_score_to_sankirtan_newsletter:true,
    group_type: "Temple",
    parent_group: {
      [Sq.Op.ne]: null,
    },
...
arrangeArrayIntoBookTypes = (pointsArr) => {
 // console.log('pointsAttay', pointsArr);
  ...
  pointsArr.map((points) => {
    if (points && points.length) {
        pointsByBookType = points.map((point, indx) => {
        return parseFloat(point) + parseFloat(pointsByBookType[indx]);
      });
    }
  });
  let continent_data = { points: getArrayValueSum(pointsByBookType) };
...
getCongregationalPreaching = async (year, period_id) => {
  ...
  console.log('congregationalSummary',congregationalSummary)
  console.log('congregationalSummary',congregationalSummary)
  const formattedResult = [];
...
const getDashboardGrowth = async (_where) => {
  ...
  if (parseInt(_where.year) >= 1965) {
    ...
    let book_progress = (growth[0].dataValues.total_annual_actual_book_points) * 100 /
...
const  updateYearly= async (
  ...
  if (
    ...
    _where = { ..._where, distributor_id: distributor_id };
    goal_yearly = {...goal_yearly,distributor_id: distributor_id,}
  }else{
    _where = { ..._where, distributor_id: null };
  }
  _where = { ..._where,period_id : null };
  const yearly_data=await BusinessPlanSummary.findOne({ where: _where });
  if(yearly_data == null){
    goal_yearly = {
      ...goal_yearly,
      year: year,
    ...
    await BusinessPlanSummary.create(goal_yearly);
  }else{
    if(type =='no_roll_up'){
      goal_yearly = {
        ...goal_yearly,
        book_points_goal: adjustExistingGoal(
          yearly_data.book_points_goal,
          goal_book_points,
          existing_data.old_book_point_goal,
        ),
        bbt_amount_goal: adjustExistingGoal(
          yearly_data.bbt_amount_goal,
          goal_amount_remitted_bbt,
          existing_data.old_bbt_amount_goal,
        ),
        group_amount_goal: adjustExistingGoal(
          yearly_data.group_amount_goal,
          goal_amount_collected,
          existing_data.old_group_amount_goal,
        ),
      }
      ...
      goal_yearly = {
        ...
        book_points_goal_roll_up: adjustExistingGoal(
          yearly_data.book_points_goal_roll_up,
          goal_book_points,
          existing_data.old_book_point_goal,
        ),
        bbt_amount_goal_roll_up: adjustExistingGoal(
          yearly_data.bbt_amount_goal_roll_up,
          goal_amount_remitted_bbt,
          existing_data.old_bbt_amount_goal,
        ),
        group_amount_goal_roll_up: adjustExistingGoal(
          yearly_data.group_amount_goal_roll_up,
          goal_amount_collected,
          existing_data.old_group_amount_goal,
        ),
      }
    ...
    goal_yearly = {
      ...
      book_points_goal_total: 
        adjustExistingGoal(
          yearly_data.book_points_goal_total,
          goal_book_points,
          existing_data.old_book_point_goal,
        ),
      ...
      bbt_amount_goal_total: 
        adjustExistingGoal(
          yearly_data.bbt_amount_goal_total,
          goal_amount_remitted_bbt,
          existing_data.old_bbt_amount_goal,
        ),
      group_amount_goal_total: 
        adjustExistingGoal(
          yearly_data.group_amount_goal_total,
          goal_amount_collected,
          existing_data.old_group_amount_goal,
        ),
      last_modified_by_id: created_by_id,
    ...
    await BusinessPlanSummary.update(goal_yearly, {
      where: { id: yearly_data.id },
    });
