...
getAllTeamTypeTransaction = async (year, period, param) => {
  ...
  const allTeamSummary = await BusinessPlanSummary.findOne({
    where: _where,
    attributes: [
      "id",
      "year",
  ...
  {
  let team_points=allTeamSummary.actual_team_book_points_book_type_total;
  SANKIRTAN_GROUP_TYPE.map((key,value) => {
...
router.get("/newsletter-team-type-transactions", async function (req, res) {
  ...
  if(returnedData.period_id != null && returnedData.last_year_period == null ){
    ...
    if(query_param.param !== undefined){
      lastYearData = await getAllTeamTypeTransaction(parseInt(returnedData.year) - 1, returnedData.last_year_period, "c");
    }
    else {
...
getTopIndividuals = async (year,period,field, param) => {
  let _where = {}
  if(param !== undefined) {
    if(period.length !== 0)
    {
